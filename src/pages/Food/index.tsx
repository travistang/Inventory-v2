...
const FoodPage: React.FC<FoodPageProps> = () => {
  ...
  const foods = data.foods.sort((fa: Food, fb: Food) =>
  ...
  return (
    ...
    >
      <SearchList
        animated
        list={foods}
        filterFunc={(food, text) =>
          (food as Food).name.toLowerCase().includes(text.toLowerCase())
        }
        inputConfig={{
        ...
        renderItem={(food) => (
          <FoodCard
            {...food}
            onClick={() =>
              history.replace(Routes.FOOD_DETAILS + `&food=${food.name}`)
            }
          />
        )}
      ></SearchList>
